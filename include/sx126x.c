...
#define delay(ms) vTaskDelay(ms/portTICK_PERIOD_MS)//esp_rom_delay_us(ms*1000)
...
void LoRaInit(lora_gpio_t *pins)
{


	ESP_LOGI(TAG, "CONFIG_MISO_GPIO=%d", pins->CONFIG_MISO_GPIO);
	ESP_LOGI(TAG, "CONFIG_MOSI_GPIO=%d", pins->CONFIG_MOSI_GPIO);
	ESP_LOGI(TAG, "CONFIG_SCLK_GPIO=%d", pins->CONFIG_SCLK_GPIO);
	ESP_LOGI(TAG, "CONFIG_NSS_GPIO=%d", pins->CONFIG_NSS_GPIO);
	ESP_LOGI(TAG, "CONFIG_RST_GPIO=%d", pins->CONFIG_RST_GPIO);
	ESP_LOGI(TAG, "CONFIG_BUSY_GPIO=%d", pins->CONFIG_BUSY_GPIO);
	ESP_LOGI(TAG, "CONFIG_TXEN_GPIO=%d", pins->CONFIG_TXEN_GPIO);
	ESP_LOGI(TAG, "CONFIG_RXEN_GPIO=%d", pins->CONFIG_RXEN_GPIO);

	SX126x_SPI_SELECT = pins->CONFIG_NSS_GPIO;
	SX126x_RESET = pins->CONFIG_RST_GPIO;
	SX126x_BUSY	= pins->CONFIG_BUSY_GPIO;
	SX126x_TXEN	= pins->CONFIG_TXEN_GPIO;
	SX126x_RXEN	= pins->CONFIG_RXEN_GPIO;
	...
	spi_bus_config_t spi_bus_config = {
		.sclk_io_num = pins->CONFIG_SCLK_GPIO,
		.mosi_io_num = pins->CONFIG_MOSI_GPIO,
		.miso_io_num = pins->CONFIG_MISO_GPIO,
		.quadwp_io_num = -1,
		.quadhd_io_num = -1
	};
	...
	spi_device_interface_config_t devcfg = {
		...
		.mode = 0,
		.spics_io_num = pins->CONFIG_NSS_GPIO,
		.queue_size = 7,
		.flags = 0,
		.pre_cb = NULL
	};
...
int16_t LoRaBegin(lora_begin_params_t *params) 
{
	if ( params->txPowerInDbm > 22 )
		params->txPowerInDbm = 22;
	if ( params->txPowerInDbm < -3 )
		params->txPowerInDbm = -3;
	...
	ESP_LOGI(TAG, "tcxoVoltage=%f", params->tcxoVoltage);
	// set TCXO control, if requested
	if(params->tcxoVoltage > 0.0) {
		SetDio3AsTcxoCtrl(params->tcxoVoltage, RADIO_TCXO_SETUP_TIME); // Configure the radio to use a TCXO controlled by DIO3
	}
	...
	ESP_LOGI(TAG, "useRegulatorLDO=%d", params->useRegulatorLDO);
	if (params->useRegulatorLDO) {
	...
	SetPowerConfig(params->txPowerInDbm, SX126X_PA_RAMP_200U); //0 fuer Empfaenger
	SetRfFrequency(params->frequencyInHz);
	return ERR_NONE;
}
...
void LoRaConfig(lora_config_params_t *config) 
{
	...
	SetModulationParams(config->spreadingFactor, config->bandwidth, config->codingRate, ldro);
	
	PacketParams[0] = (config->preambleLength >> 8) & 0xFF;
	PacketParams[1] = config->preambleLength;
	if ( config->payloadLen )
	{
		PacketParams[2] = 0x01; // Fixed length packet (implicit header)
		PacketParams[3] = config->payloadLen;
	}
	else
	{
