...
/**
* @brief struct that defines what pins are gona be used for communication with lora module
*
*
* @param CONFIG_MISO_GPIO miso
* @param CONFIG_MOSI_GPIO mosi
* @param CONFIG_SCLK_GPIO clock
* @param CONFIG_NSS_GPIO  chip sellect pin
* @param CONFIG_RST_GPIO  sx216x reset pin
* @param CONFIG_BUSY_GPIO sx126x busy pin
* @param CONFIG_TXEN_GPIO active high tx enable pin (-1 if not in use)
* @param CONFIG_RXEN_GPIO active high rx enable pin (-1 if not in use)
*
*/
typedef struct 
{
    int CONFIG_MISO_GPIO;
    int CONFIG_MOSI_GPIO;
    int CONFIG_SCLK_GPIO;
    int CONFIG_NSS_GPIO;
    int CONFIG_RST_GPIO;
    int CONFIG_BUSY_GPIO;

    int CONFIG_TXEN_GPIO;
    int CONFIG_RXEN_GPIO;
} lora_gpio_t;

/**
* @brief struct to set up initial paramiters for lora to start 
*
* @param frequencyInHz self explanetory
* @param txPowerInDbm  also self explanetory
* @param tcxoVoltage   voltage for tcxo, has to be looked up in documentation of a module, defined in volts,
*                      if u dont use tcxo, set to 0.0
* @param useRegulatorLDO set to true if useing Ldo, false if dc-dc
*/
typedef struct
{
...
/**
* @brief settings for lora comunication 
*
* @param bandwidth set 4 for 125k (look up documentatnion for other settings)
* @param codingRate set 1 for 4/5 (look up documentatnion for other settings)
* @param preambleLength ...
* @param payloadLen set 0 for automatic packet size setting every send event
*/
typedef struct
{
...
/**
* @brief function starting comunication with lora module and defineing connection pins
*
* @param pins pointer to struct with pin definitions
*/
void     LoRaInit(lora_gpio_t *pins);

/**
* @brief function setting up some of the nessesery paramiters to start comunication
*
* @param params pointer to struct with paramiters definitions
*
* @return returns 0 if begined corectly
*/
